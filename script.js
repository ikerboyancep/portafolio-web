...
function createProjectsGrid() {
  ...
  proyectos.forEach((p) => {
    ...
    // Build card content with a programmatically-created iframe that includes enablejsapi and origin
    const title = document.createElement('h3');
    title.style.cssText = "font-family:'Cinzel',serif;color:" + TEXT_COLOR + ";text-align:center;";
    title.textContent = p.titulo;

    const vidWrap = document.createElement('div');
    vidWrap.style.cssText = 'position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:8px;';

    const iframe = document.createElement('iframe');
    // Ensure enablejsapi and origin are provided so the IFrame API can communicate without origin errors
    let src = p.video || '';
    if (src.indexOf('enablejsapi=1') === -1) {
      src += (src.indexOf('?') === -1 ? '?' : '&') + 'enablejsapi=1&rel=0&origin=' + encodeURIComponent(location.origin);
    }
    iframe.src = src;
    ...
    desc.style.cssText = "font-family:'Cinzel',serif;color:" + ACCENT_COLOR + ";font-size:0.9rem;text-align:center;margin-top:10px;";
...
/*==============================================================*\
 |  🧩 Grid dinámico de proyectos con videos de YouTube
\*==============================================================*/
function createProjectsGrid() {
  const grid = document.createElement("div");
  Object.assign(grid.style, {
  ...
  const proyectos = [
    ...
    },
    {
      ...
      video: "https://www.youtube.com/embed/JoyD_b_xtUA",
      descripcion:
        "Menú principal del proyecto Uri. Ambientación sonora, programación y efectos visuales de desarrollo propio.",
    },
  ];
  ...
  proyectos.forEach((p) => {
    ...
    card.innerHTML = `
      <h3 style="font-family:'Cinzel',serif;color:${TEXT_COLOR};text-align:center;">${p.titulo}</h3>
      <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:8px;">
        <iframe src="${p.video}" frameborder="0" allow="autoplay; fullscreen" allowfullscreen
                style="position:absolute;top:0;left:0;width:100%;height:100%;"></iframe>
      </div>
      <p style="font-family:'Cinzel',serif;color:${ACCENT_COLOR};font-size:0.9rem;
                text-align:center;margin-top:10px;">${p.descripcion}</p>
    `;
    grid.appendChild(card);
  });
